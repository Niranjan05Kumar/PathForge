...
import { validateGenerateGraphRequest } from '../validation/benchmarkValidator';
import { executeEngine } from '../engine/engineBridge';

export const graphRouter = Router();

async function handleGenerate(req: Request, res: Response) {
  const validation = validateGenerateGraphRequest(req.body);
  if (!validation.valid) {
  ...
  try {
    const payload = {
      ...
      graphConfig: req.body.graphConfig || req.body
    };

    const engineResponse = await executeEngine(payload, 20000);
    ...
    return res.status(500).json({
      ...
      error: {
        ...
        message: err.message || 'An unexpected error occurred generating synthetic graph.'
      }
    });
  }
}

graphRouter.post('/generate', handleGenerate);
graphRouter.post('/', handleGenerate);
